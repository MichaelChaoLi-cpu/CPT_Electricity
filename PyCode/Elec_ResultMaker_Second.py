...
from Elec_Encoder import Encoder_FullContentMaker
...
if __name__ == '__main__':
    ...
    number_done_list = []
    for file_name in DoneFile:
            match = re.search(r'(\d+)\.pdf', file_name)
            if match:
                number_done_list.append(match.group(1))
    ...
    for file in file_need_run:
        if file[-5] != ')':
            file_to_use.append(file)

    file_to_use_number_search = []
    for file_name in file_to_use:
        match = re.search(r'(\d+)\.pdf', file_name)
        if match:
            if match.group(1) in number_done_list:
                file_to_use_number_search.append(file_name)
    
    file_to_use_sorted = sorted(file_to_use_number_search)[Begin_Point: End_Point]
    result_dict_list = []
    for i, single_file in enumerate(file_to_use_sorted):
        ...
        try:
            ...
            result_dict['model_i0_index'] = np.mean(model_list[0].predict(dataX, verbose=0)[:,1])
            result_dict['model_i1_index'] = np.mean(model_list[1].predict(dataX, verbose=0)[:,1])
            result_dict['model_i2_index'] = np.mean(model_list[2].predict(dataX, verbose=0)[:,1])
            result_dict['model_i3_index'] = np.mean(model_list[3].predict(dataX, verbose=0)[:,1])
            result_dict['model_i4_index'] = np.mean(model_list[4].predict(dataX, verbose=0)[:,1])
            result_dict['model_i5_index'] = np.mean(model_list[5].predict(dataX, verbose=0)[:,1])
            result_dict['model_i6_index'] = np.mean(model_list[6].predict(dataX, verbose=0)[:,1])
            result_dict['model_i7_index'] = np.mean(model_list[7].predict(dataX, verbose=0)[:,1])
            result_dict['model_i8_index'] = np.mean(model_list[8].predict(dataX, verbose=0)[:,1])
            result_dict['model_i9_index'] = np.mean(model_list[9].predict(dataX, verbose=0)[:,1])
            result_dict['model_i10_index'] = np.mean(model_list[10].predict(dataX, verbose=0)[:,1])
            result_dict['model_i11_index'] = np.mean(model_list[11].predict(dataX, verbose=0)[:,1])
            result_dict['model_i12_index'] = np.mean(model_list[12].predict(dataX, verbose=0)[:,1])
            dataX = None
            print(result_dict)
        except:
